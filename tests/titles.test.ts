...
test('titles rank champion over survivor over ironfoot over pioneer', () => {
  ...
  assert.equal(titleFor({ crowns: 10, streak: 0, finaleWins: 0, villager: true }), 'VILLAGER')
  assert.equal(titleFor({ crowns: 10, streak: 3, finaleWins: 0, villager: true }), 'SURVIVOR')
})
